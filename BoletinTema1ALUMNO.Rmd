...
#A
  #Mammals= a data frame with average brain and body weights for 62 species of land mammals.
  #Animals= Average brain and body weights for 28 species of land animals.
...
commonAnimals<- intersect(rownames(Mammals), rownames(Animals))
...
```{r}
diferentes1<-setdiff(rownames(Mammals), rownames(Animals))
diferentes1
diferentes2<-setdiff(rownames(Animals), rownames(Mammals))
diferentes2

total<- c(diferentes1, diferentes2)
total
...
e.  Determina las diferencia entre los animales que no aparecen en ambos conjuntos
    ...
    \-\-\--Muchos animales que no aparecen en ambos conjuntos son diniosaurios (aparecen en Animals -----pero no en el df de Mammals. Otros es porque se han escrito erroneamente/diferente (ej: Patas ----monkey/Patar Monkey)
...
qqPlot(log(Animals$brain))

#Presentan una distrubución muy parecida, en ambos casos no hay ningun punto que salga de la zona azul, por lo que  siguen una distribución Gaussiana con un 95 % de confianza
...
3.  La función **library** sin argumentos abre una ventana y muestra las librerías que han sido instaladas.
    ...
    length(libReturn$results[, 1]) #todas las filas, primera columna(título)
    #184 librerias
...
hist(cabbages$VitC[cabbages$Cult == "c39"], 
     main = "Histograma del contenido de vitamina C para la variedad c39", 
     xlab = "Contenido de vitamina C", 
     col = "green",
     breaks = 10)
hist(cabbages$VitC[cabbages$Cult == "c52"], 
     main = "Histograma del contenido de vitamina C para la variedad c52", 
     xlab = "Contenido de vitamina C", 
      col="yellow", 
     breaks = 10, add=TRUE, alpha=(0.5))
...
plot(whiteside$Temp, whiteside$Gas, pch=c(6,16)[whiteside$Insul]) #barra??
...
legend(x="topright",legend=c("Cult = c39","Insul = c52"), pch=c(6,16))
# Planteamos 2 modelos lineales, uno para los datos de cada factor
Model1 <- lm(cabbages$VitC~cabbages$HeadWt, data= cabbages, subset= which(cabbages$Cult=="c39"))
Model2 <- lm(cabbages$VitC~cabbages$HeadWt, data= cabbages, subset= which(cabbages$Cult=="c39"))
# Representamos las rectas correspondientes a cada modelo lineal
abline(Model1, lty=2)
abline(Model2)
